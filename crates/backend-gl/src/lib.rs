...
pub use backend::*; // neutral descriptor types now live in the `backend` crate
...
/// The `Backend` seam for the GL renderer — a thin adapter over the inherent
/// methods (which stay inherent so the `main.rs` diagnostics can keep calling
/// them on a `GlBackend` directly).
impl backend::Backend for GlBackend {
    fn present_windows(
        &self,
        items: &[WindowDraw],
        screen_w: i32,
        ...
        GlBackend::present_windows(self, items, screen_w, screen_h, hud, osd, clear)
    }
    fn set_render_params(&mut self, render: RenderParams) {
        GlBackend::set_render_params(self, render)
    }
    fn set_font(&mut self, path: &str, size: f32) {
        GlBackend::set_font(self, path, size)
    }
    fn has_text(&self) -> bool {
        GlBackend::has_text(self)
    }
    fn render_ms(&self) -> f32 {
        GlBackend::render_ms(self)
    }
